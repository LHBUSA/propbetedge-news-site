 ...
 * src/ads-config.js
 * -------------------------------------------------------------------------
 * ALL ad slots and creatives defined here. FTC-compliant with disclosure.
 *
 * v3.16: Added "Advertisement" disclosure to every ad slot (FTC compliance)
 *        Added rel="sponsored" to all ad links (Google 2019 spec)
 *        Added 1-800-GAMBLER inline tag on sportsbook ads (state compliance)
 *        Added same-page frequency cap (no back-to-back same brand)
 *        ASCII-safe source (avoids PowerShell unzip mangling)
 *
 * v3.9.2: 50/50 brand/sportsbook split in mid-article
 * -------------------------------------------------------------------------
 */
...
export const PROPBET_LINKS = {
  ...
  discord:     'https://discord.gg/e9S6pFq9',
...
// Module-level state means it persists across calls in the same render pass.
    ...
    <div class="ad-wrapper" data-ad-slot="${slotName}" data-ad-brand="${brand.key}">
      ...
      <a href="${trackedHref}" class="ad-block ad-brand-family ad-tone-${brand.tone}" target="_blank" rel="noopener sponsored">
        <div class="ad-block-content">
          ...
          <span class="ad-block-cta">${brand.cta} &rarr;</span>
        </div>
      </a>
    </div>
  `;
  ...
  const sportsbookCreatives = [
    {
      ...
      sub: '21+ &middot; T&Cs apply &middot; See sportsbook for details &middot; If you or someone you know has a gambling problem, call 1-800-GAMBLER',
      cta: 'Open DraftKings',
    ...
    },
    {
      ...
      sub: '21+ &middot; T&Cs apply &middot; If you or someone you know has a gambling problem, call 1-800-GAMBLER',
      cta: 'Open FanDuel',
    ...
    <div class="footer-cta">
      <div class="container footer-cta-inner">
        <div class="footer-cta-text">
          <span class="footer-cta-eyebrow">\u26A1 Tonight's Slate &middot; Every League</span>
          <h3 class="footer-cta-headline">Sharp picks. AI-built. Built to win.</h3>
          <p class="footer-cta-sub">The same model that wrote this article grades picks against live odds 24/7. Pick your sport &rarr;</p>
        </div>
    ...
    <div class="ad-banner-wrapper" data-ad-slot="header_banner">
      ...
      <a href="${href}" class="ad-banner ad-tone-${tone}" target="${isExternal ? '_blank' : '_self'}" rel="noopener sponsored">
        <div class="ad-banner-inner">
          ...
          <span class="ad-banner-cta">${cta} &rarr;</span>
        </div>
      </a>
    </div>
  `;
...
function renderAdBlock({ tone, eyebrow, headline, sub, cta, href, sportsbook }) {
  ...
  const disclosureLabel = isSportsbook ? 'Advertisement &middot; 21+' : 'Advertisement';
  return `
    <div class="ad-wrapper" data-ad-slot="mid_article"${sportsbook ? ` data-ad-sportsbook="${sportsbook}"` : ''}>
      ...
      <a href="${href}" class="ad-block ad-tone-${tone}" target="${isExternal ? '_blank' : '_self'}" rel="noopener sponsored">
        <div class="ad-block-content">
          <span class="ad-block-eyebrow">${eyebrow}${sportsbook ? ` &middot; Sponsored` : ''}</span>
          <h3 class="ad-block-headline">${headline}</h3>
          ${sub ? `<p class="ad-block-sub">${sub}</p>` : ''}
          <span class="ad-block-cta">${cta} &rarr;</span>
        </div>
      </a>
    </div>
  `;
