...
function clearAndRoute() {
  ...
  if (path === '/' || path === '') {
    setMeta({
      title: 'PropBetEdge ΓÇö Sports News & Prop-Bet Intelligence',
  ...
  if (path === '/news') {
    setMeta({
      title: 'All Sports News ΓÇö PropBetEdge',
  ...
  if (sportMatch) {
    ...
    setMeta({
      title: `${sportLabel} News ΓÇö PropBetEdge`,
  ...
  // ≡ƒåò Author profile: /authors/:slug
  ...
  // ≡ƒåò Editorial Standards page (referenced by Org schema policy URLs)
  if (path === '/editorial-standards') {
    return renderEditorialStandards(root, setMeta);
  }

  setMeta({ title: 'Not found ΓÇö PropBetEdge', description: 'Page not found.' });
